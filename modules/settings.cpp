...
#define CLTOTAB_RECT_OFFS_LEFT   10
#define CLTOTAB_RECT_OFFS_TOP    50
#define CLTOTAB_RECT_OFFS_RIGHT  10
#define CLTOTAB_RECT_OFFS_BOTTOM 16
...
void SettingsSetDefaultsEnhanced(json::JSON& jsonSettings) {
	...
	jsonSettings[C_SC2KFIX][S_FIX_AUDIO][I_FIX_AUD_SHUFFLEMUSIC] = true;
	...
	jsonSettings[C_SC2KFIX][S_FIX_AUDIO][I_FIX_AUD_ALWAYSPLAYMUSIC] = true;
	...
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_DARKUNDGRND] = true;
	...
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_USEFLTSTATUS] = true;
...
void SettingsSetDefaultsVanillaPlus(json::JSON& jsonSettings) {
	jsonSettings[C_SC2KFIX][S_FIX_CORE][I_FIX_CORE_FORCECON] = DEF_FIX_CORE_FORCECON;
	jsonSettings[C_SC2KFIX][S_FIX_CORE][I_FIX_CORE_CHECKFORUPD] = DEF_FIX_CORE_CHECKFORUPD;
	jsonSettings[C_SC2KFIX][S_FIX_CORE][I_FIX_CORE_SKIPMODS] = DEF_FIX_CORE_SKIPMODS;
...
void SettingsSetDefaultsVanilla(json::JSON& jsonSettings) {
	jsonSettings[C_SC2KFIX][S_FIX_CORE][I_FIX_CORE_FORCECON] = DEF_FIX_CORE_FORCECON;
	jsonSettings[C_SC2KFIX][S_FIX_CORE][I_FIX_CORE_CHECKFORUPD] = DEF_FIX_CORE_CHECKFORUPD;
	jsonSettings[C_SC2KFIX][S_FIX_CORE][I_FIX_CORE_SKIPMODS] = DEF_FIX_CORE_SKIPMODS;
	...
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_FREQUPDATES] = false;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_WEATHEREFFECTS] = false;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_DARKUNDGRND] = DEF_FIX_QOL_DARKUNDGRND;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_SKIPINTRO] = DEF_FIX_QOL_SKIPINTRO;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_USENEWSTRINGS] = false;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_USEFLTSTATUS] = DEF_FIX_QOL_USENEWSTRINGS;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_TITLECALEND] = false;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_MOVZOOMFACTOR] = 1;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_LEGACYCTYTRNCSM] = DEF_FIX_QOL_LEGACYCTYTRNCSM;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_TERRAINCOSMETIC] = DEF_FIX_QOL_TERRAINCOSMETIC;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_SC2K_SAVECITYBK] = false;
	jsonSettings[C_SC2KFIX][S_FIX_QOL][I_FIX_QOL_NEWCITYVARIETY] = false;
...
void DefaultSettingsSC2KFixCore(json::JSON& jsonSettings) {
	SettingsSetDefaultsVanillaPlus(jsonSettings);
...
void LoadJSONSettings(void) {
	...
	if (fSettingsJSON.good()) {
		...
		if (strLoadedJSONDump.good()) {
			...
			jsonSettingsCore.merge(json::Load(strLoadedJSONDump.str()));
		}
	}
...
static BOOL CALLBACK SettingsDialogGeneralTabProc(HWND hwndDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
		...
		pRect = &stSettingsDialogHeader.rcTab;
		SetWindowPos(hwndDlg, NULL, pRect->left, pRect->top,
...
BOOL CALLBACK SettingsDialogContainerProc(HWND hwndDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
		...
		pRect = &stSettingsDialogHeader.rcTab;
		CopyRect(pRect, &rcTab);
		ScreenToClient(hwndDlg, (LPPOINT)&pRect->left);
		ScreenToClient(hwndDlg, (LPPOINT)&pRect->right);
		SetRect(pRect, pRect->left + CLTOTAB_RECT_OFFS_LEFT, pRect->top + CLTOTAB_RECT_OFFS_TOP, 
			pRect->right - CLTOTAB_RECT_OFFS_RIGHT, pRect->bottom - CLTOTAB_RECT_OFFS_BOTTOM);
	...
	case WM_COMMAND:
		...
		case IDC_SETTINGS_BUTTON_DEFAULTS:
			// Set all the checkboxes to the defaults (the "vanilla+" install option).
			SettingsSetDefaultsVanillaPlus(jsonSettingsCoreWorkingCopy);

			// Refresh the tab we're on
			SettingsTabSelectionChanged(hwndDlg);
			break;
		case IDC_SETTINGS_BUTTON_VANILLA:
			// Reset all the checkboxes to the "vanilla" install option.
			SettingsSetDefaultsVanilla(jsonSettingsCoreWorkingCopy);
